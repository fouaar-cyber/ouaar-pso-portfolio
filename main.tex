...
\author{Safia Ouaar$^{a}$ and Fatima Ouaar$^{b}$\\
	...
	\small \texttt{f.ouaar@univ-biskra.dz}}
...
\begin{document}
	...
	\begin{abstract}
		\noindent \textbf{Purpose:} This study proposes a utility-feedback particle swarm optimiser (PSO) that adaptively modulates search persistence using realised portfolio utility, addressing ad-hoc parameter tuning in higher-moment portfolio optimisation.\\
		\textbf{Design/Methodology/Approach:} We employ shrinkage estimation for covariance matrices and factor-structured higher moments, with utility weights sensitivity analysis. Experiments cover S\&P 500 (2005--2023) and Borsa Istanbul BIST-100 (2010--2023) using block-bootstrap inference.\\
		\textbf{Findings:} Adaptive PSO achieves modest improvements over fixed-$\omega$ PSO on the S\&P 500 (Sharpe 1.12 vs 1.09). In BIST-100, the 1/N equal-weight benchmark dominates both PSO methods, suggesting that estimation error likely offsets potential gains from optimization. Results are robust to transaction costs up to 15 basis points.\\
		\textbf{Originality/Value:} We provide open-source reproducible code and demonstrate that utility-feedback mechanisms adaptively modulate search persistence, though their efficacy depends on market development and estimation precision.\\
		\textbf{Keywords:} Portfolio optimisation; Higher-order moments; Particle swarm optimisation; Utility feedback; Shrinkage estimation; Bootstrap inference\\
		\textbf{JEL Classification:} G11; C61; C63; C15
	\end{abstract}
	
	\section{Introduction}
	
	Investor preferences have shifted from mean--variance to higher-moment criteria, reflecting recognition that skewness and kurtosis affect portfolio utility \citep{harvey2020conditional,scott1980preference,jonsson2023moment}. However, polynomial goal-programming remains non-convex and estimation-error sensitive \citep{maringer2009global,ledoit2004well}.
	
	Meta-heuristics---including particle swarm optimisation (PSO)---offer alternatives to convex relaxations \citep{golmakani2022constrained,zhang2020portfolio}. Yet two gaps persist: (i) inertia weight tuning remains largely ad-hoc \citep{poli2007pso}, and (ii) higher-moment estimation error is rarely addressed \citep{jonsson2023moment}.
	...
	\textbf{Covariance shrinkage} \citep{ledoit2004well}:
	\begin{equation}
	...
	\label{eq:omega}
	\end{equation}
	with $\epsilon=10^{-6}$ preventing division by zero, and $\omega_k$ clamped to $[0.4,0.9]$. This interval is widely used in PSO practice to balance stability and search mobility \citep{poli2007pso}.
	...
	Following \citet{ledoit2008robust}, we employ circular block bootstrap with block length $b=21$ days (matching the monthly rebalancing frequency). For each of $B=1000$ bootstrap samples, we recompute Sharpe ratios and construct percentile 95\% confidence intervals.
	...
	\begin{figure}[!t]
		\centering
		\includegraphics[width=0.9\textwidth]{figure1_sharpe_comparison.pdf}
	...
	\begin{figure}[!t]
		\centering
		\includegraphics[width=0.9\textwidth]{figure2_cumulative_returns.pdf}
	...
	\begin{figure}[!t]
		\centering
		\includegraphics[width=0.9\textwidth]{figure4_sharpe_distribution.pdf}
	...
	\begin{figure}[!t]
		\centering
		\includegraphics[width=0.9\textwidth]{sensitivity_heatmap.pdf}
	...
	\begin{figure}[!t]
		\centering
		\includegraphics[width=0.9\textwidth]{figure3_omega_evolution.pdf}
	...
	\begin{figure}[!t]
		\centering
		\includegraphics[width=0.9\textwidth]{figure5_transaction_cost.pdf}
	...
	All data used in this study are publicly available. S\&P 500 and BIST-100 stock returns were obtained from Yahoo Finance via the \texttt{yfinance} Python library (\url{https://pypi.org/project/yfinance/}). Replication code will be made available at \url{https://github.com/[username]/[repository]} upon paper acceptance.
	...
	\begin{appendices}
		\section{BIST-100 Data Construction}
	...
	\begin{thebibliography}{20}
		...
		\bibitem[Eberhart \& Kennedy, 1995]{eberhart1995pso}
		Eberhart, R., \& Kennedy, J. (1995). A new optimizer using particle swarm theory. In \textit{Proceedings of the IEEE International Symposium on Micro Machine and Human Science} (pp. 39--43). IEEE.
		
		\bibitem[Golmakani \& Fazel, 2022]{golmakani2022constrained}
		Golmakani, H. R., \& Fazel, M. (2022). Constrained PSO for skewness-kurtosis portfolio models: An updated benchmark. \textit{Expert Systems with Applications}, 203, 117129.
		
		\bibitem[Harvey \& Siddique, 2020]{harvey2020conditional}
		Harvey, C. R., \& Siddique, A. (2020). Conditional skewness in asset pricing tests: Twenty-year retrospective. \textit{Journal of Finance}, 76(4), 1821--1852.
		
		\bibitem[Jonsson \& Rockinger, 2023]{jonsson2023moment}
		Jonsson, M., \& Rockinger, M. (2023). Moment component analysis in portfolio selection: A review. \textit{Journal of Banking \& Finance}, 148, 106118.
		...
		Ledoit, O., \& Wolf, M. (2008). Robust performance hypothesis testing with the Sharpe ratio. \textit{Journal of Empirical Finance}, 15(5), 850--859.
		
		\bibitem[Maringer \& Parpas, 2009]{maringer2009global}
		Maringer, D., \& Parpas, P. (2009). Global optimization of higher order moments in portfolio selection. \textit{Journal of Global Optimization}, 43(2--3), 219--230.
		
		\bibitem[Poli et al., 2007]{poli2007pso}
		Poli, R., Kennedy, J., \& Blackwell, T. (2007). Particle swarm optimization: An overview. \textit{Swarm Intelligence}, 1(1), 33--57.
		
		\bibitem[Scott \& Horvath, 1980]{scott1980preference}
		Scott, R. C., \& Horvath, P. A. (1980). On the direction of preference for moments of higher order than the variance. \textit{Journal of Finance}, 35(4), 915--919.
		
		\bibitem[Zhang et al., 2020]{zhang2020portfolio}
		Zhang, Y., Gong, D. W., \& Gao, X. Z. (2020). Portfolio optimization based on multi-objective particle swarm optimization open architecture. \textit{Expert Systems with Applications}, 151, 113371.
